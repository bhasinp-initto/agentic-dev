...
| `# Walkthrough` (added 1.4.0) | If the intent mentions any UI work (web, mobile, dashboard, frontend, button, page, route, component, etc.) emit a QUESTION-N block asking the user for `Acceptance URL`, `Dev server command`, `Dev server ready pattern` (optional regex), and an `Acceptance criteria` bullet list (natural-language steps + verifications). If the intent is clearly backend-only / library-only / CLI-only, emit the section with a single bullet `- Acceptance URL: skip` and nothing else. The walkthrough lifecycle treats `skip`/`n/a`/`none` URLs as a clean honest skip. Expected bullet shape when populated: `- Acceptance URL: http://localhost:5173/`, `- Dev server command: npm run dev`, `- Dev server ready pattern: Local:.*5173`, `- Acceptance criteria:` followed by a nested bullet list. |
